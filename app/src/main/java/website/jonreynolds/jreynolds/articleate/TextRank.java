...
public class TextRank {
    ...
    private static TextRank instance = new TextRank();
...
//                        System.out.println(weight);
    ...
    private double calculateSimilarity(SentenceVertex v1, SentenceVertex v2){
        CoreMap map1 = v1.getAnnotatedSentence();
        CoreMap map2 = v2.getAnnotatedSentence();
        //Loop through tokens in first sentence
        double similarities = 0;
        for (CoreLabel t1: map1.get(CoreAnnotations.TokensAnnotation.class)) {
            // this is the text of the token
            String word1 = t1.get(CoreAnnotations.TextAnnotation.class);
            // this is the POS tag of the token
            String pos1 = t1.get(CoreAnnotations.PartOfSpeechAnnotation.class);

            // this is the NER label of the token
//            String ne1 = t1.get(CoreAnnotations.NamedEntityTagAnnotation.class);

            //Loop through tokens in second sentence
            for (CoreLabel t2: map2.get(CoreAnnotations.TokensAnnotation.class)) {
                // this is the text of the token
                String word2 = t2.get(CoreAnnotations.TextAnnotation.class);
                //If the words are equal and nouns, verbs, or adjectives
...
//                    System.out.println(pos1 + " " + word1);
        ...
        int numWordsInSentence1 = map1.get(CoreAnnotations.TokensAnnotation.class).size();
        int numWordsInSentence2 = map2.get(CoreAnnotations.TokensAnnotation.class).size();
    ...
    public static void main(String[] args){
        ...
        tr.convergeScores();
        ArrayList<SentenceVertex> sorted = new ArrayList<SentenceVertex>();
        sorted.addAll(tr.graph.vertexSet());
        Collections.sort(sorted, new SentenceVertexComparator());
        for(SentenceVertex v: sorted){
            System.out.println(v.getScore());
            System.out.println(v.getSentence());
        }
        tr.init();
        tr.createGraph(text2);
        tr.convergeScores();
        sorted = new ArrayList<SentenceVertex>();
        sorted.addAll(tr.graph.vertexSet());
        Collections.sort(sorted, new SentenceVertexComparator());
        for(SentenceVertex v: sorted){
            System.out.println(v.getScore());
